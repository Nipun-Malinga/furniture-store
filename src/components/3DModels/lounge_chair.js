...
const loungeChair = (scene, name) => {
  ...
  cushionMat.diffuseColor = new Color3(0.9, 0.9, 0.95);

  const frameMat = new StandardMaterial('frameMat', scene);
  frameMat.diffuseColor = new Color3(0.3, 0.2, 0.15);
  ...
  back.position.y = 2;
  back.position.z = -0.9;
  back.rotation.x = Math.PI / 80;
  back.material = frameMat;
  back.parent = parent;

  const seatCushion = MeshBuilder.CreateBox(
    'seatCushion',
    { width: 2.3, depth: 2.1, height: 0.25 },
    scene
  );
  ...
  const backCushion = MeshBuilder.CreateBox(
    'backCushion',
    { width: 2.3, depth: 0.25, height: 1.7 },
    scene
  );
  backCushion.position.y = 2.05;
  backCushion.position.z = -0.85;
  backCushion.rotation.x = Math.PI / 80; 
